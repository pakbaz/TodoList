...
foreach ($cred in $credentials) {
    ...
    if ($existingCred.Count -eq 0) {
        ...
        Write-ColorOutput $Yellow "  ⚠️  Already exists: $($cred.name)"
    }
}

# Output GitHub Actions secrets/variables
Write-ColorOutput $Green "`n🎯 GitHub Repository Configuration"
...
Write-ColorOutput $Green "`n📄 Configuration saved to: $summaryFile"
